...
def generate_answer(input_query, context, router, chat_history=None, model_choice=None):
    if router == 'triet-hoc' or router == 'lich-su-dang':     
        prompt = f"""
            ...
            - Hãy thực hiện xuống dòng với những câu bắt đầu bằng a), b), c) hoặc "-" hoặc "+" hoặc in đậm để dễ đọc.
            ...
            **Tóm lại:**  
            ...
            Tóm lại: Triết học là một hệ thống tri thức lý luận toàn diện, nghiên cứu những quy luật chung nhất của thế giới tự nhiên, xã hội và tư duy. Nó giúp con người nhận thức sâu sắc về bản thân và vị trí của mình trong vũ trụ, đồng thời là sự diễn tả thế giới quan dưới dạng lý luận học thuật.\n
            Lưu ý: 
            - Giữ đúng định dạng xuống dòng, thụt đầu dòng và trình bày như ví dụ trên. Không viết liền mạch trong một đoạn duy nhất.
            - Nếu định dạng không đúng theo format trên, hãy tự format lại sao cho hợp lý và dễ đọc. Không dồn hết nội dung vào một dòng.
            """
    ...
    #     Dưới đây là query của người dùng về các câu hỏi liên quan tới Triết học:
    #     {input_query}
    ...
    #     - Hạn chế sao chép nguyên văn toàn bộ một đoạn nào từ context.
    #     - Văn phong rõ ràng, súc tích, mang tính học thuật.
    #     - Ghi rõ nguồn gốc của thông tin bằng ID của đoạn văn bản trong context, đặt ID trong ngoặc vuông ở cuối câu, ID này chính là id của các chunk trong context.
    #     - Chỉ trả lời theo nội dung context cung cấp. Nếu những nội dung đó không liên quan đến câu hỏi thì trả lời: **"Context không liên quan."**
    #     - Hãy thực hiện xuống dòng với những câu bắt đầu bằng a), b), c) hoặc "-" hoặc in đậm để dễ đọc.
